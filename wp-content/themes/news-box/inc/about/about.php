...
function news_box_notice__error() {
	...
	$message = __( '<strong><span style="color:red"> Hi Buddy!! You are using the free version. If you are happy with the free version then good but If you want a beautiful, orderly, SEO friendly, more secure and unlimited options website then need to upgrade pro. UPGRADE FOR LIFETIME WITH NOMINAL PRICE.</span><br> <span style="color:green;"> PRO License Now Lifetime For You. Buy One Time And Get Lifetime Update, High-Quality performance And video posts feature. So Why You late? Build Your Site With Pro Version.</span> <span style="color:red;"><a target="_blank" href="https://wpthemespace.com/product/news-box/">So buy pro today</a></span> </strong> ', 'news-box' );
    $url1 = esc_url('https://wpthemespace.com/product/news-box/');
    $url2 =esc_url('http://wpthemespace.com/theme/news-box/');
    $url3 =esc_url('https://wpthemespace.com/product/x-blog/');

	printf( '<div class="%1$s" style="padding:10px 15px 20px;text-transform:uppercase"><p>%2$s</p><a target="_blank" class="button button-danger" href="%3$s" style="margin-right:10px">'.esc_html('View Details & Buy').'</a><a target="_blank" class="button button-primary" href="%4$s">'.esc_html('News Box Demo').'</a><a style="margin:0 10px" target="_blank" class="button button-primary" href="%5$s">'.esc_html('View most popular XBLOG Pro').'</a><a class="eyehide" href="#">'.esc_html('Dismiss the notice').'</a></div>', esc_attr( $class ), wp_kses_post( $message ),$url1,$url2,$url3 ); 
}
add_action( 'admin_notices', 'news_box_notice__error' );

function news_box_notice_option(){
    if(isset($_GET['hnotice']) && $_GET['hnotice'] == 1 ){
        update_option( 'nbox5', 1);
    }
}
