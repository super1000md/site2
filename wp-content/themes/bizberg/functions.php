...
add_action("wp_ajax_bizberg_install_plugins", "bizberg_install_plugins");
function bizberg_install_plugins(){
	...
	$recommended_plugins = array(
		...
		'elegant-blocks' => array(
			'slug' => 'elegant-blocks/plugin.php',
			'zip' => 'https://downloads.wordpress.org/plugin/elegant-blocks.latest-stable.zip'
		),
	...
	foreach ( $recommended_plugins as $key => $value ) {
		...
		if( in_array( $key, $install_activate_plugins ) ){
			...
			if ( !bizberg_is_plugin_installed( $value['slug'] ) ) {
		    	bizberg_install_plugin( $value['zip'] );
		  	}
